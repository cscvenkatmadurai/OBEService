...
public class QuestionResource {
	...
	public Response getQuestions(@PathParam("courseID") final String courseId,@PathParam("year") final int year, @PathParam("assessmentId") final String assessmentId) {

		List<Question> questionsList;

		try {
			questionsList = QuestionRelation.getQuestions(courseId,assessmentId,year);
	...
	@DELETE
	@Path("/{courseId}/{year}")
	public Response delete(@PathParam("courseId") final String courseId,@PathParam("year") final int year){
		try {
			
			boolean status = QuestionRelation.deleteQuestionCourseIdYear(courseId,year);
			if(status){return Response.status(Status.OK)
					.header("Access-Control-Allow-Origin", "*").build();
			}else{
				return Response.status(Status.BAD_REQUEST).header("Access-Control-Allow-Origin", "*").build();
			}
	...
	@DELETE
	@Path("/{courseId}/{year}/{assessmentId}")
	public Response delete(@PathParam("courseId")  String courseId,@PathParam("year")  int year,@PathParam("assessmentId")  String assessmentId){
		try {
			
			boolean status = QuestionRelation.deleteQuestionCourseIdYearAssessmentId(courseId, year, assessmentId);
			if(status){
	...
	public Response update(final Question question,@PathParam("courseId")final String courseId , @PathParam("year") final int year,@PathParam("assessmentId") final String assessmentId,@PathParam("questionNo") final String questionNO,@PathParam("section") final String section){
		try {
			boolean status = QuestionRelation.update(question, courseId, year, assessmentId, questionNO, section);
			if(status){
				return Response.status(Status.OK).header("Access-Control-Allow-Origin", "*").build();
			}else{
				return Response.status(Status.BAD_REQUEST).header("Access-Control-Allow-Origin", "*").build();
